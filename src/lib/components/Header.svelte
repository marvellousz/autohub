            ...
            <ul class={`hidden md:flex space-x-2 items-center lg:space-x-4 ${isMobileMenuOpen ? '!flex flex-col fixed inset-x-0 top-[88px] bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-6 space-y-4 shadow-2xl border-t border-primary-200 dark:border-primary-800 z-50 rounded-b-2xl' : ''}`}>
                ...
                {#if $authStore.isAuthenticated}
                    <li class="relative group md:w-auto w-full">
                        <button class="flex items-center text-primary-700 dark:text-primary-300 hover:text-primary-600 dark:hover:text-primary-400 focus:outline-none px-4 py-3 rounded-xl hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-all duration-300 w-full md:w-auto group">
                        ...
                        <div class="{isMobileMenuOpen ? 'relative md:absolute w-full mt-2' : 'absolute'} right-0 w-56 py-3 mt-2 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md rounded-2xl shadow-2xl {isMobileMenuOpen ? 'block' : 'invisible group-hover:visible'} z-10 border border-primary-200 dark:border-primary-800 transition-all duration-300">
                            <a href="/profile" class="flex items-center px-6 py-3 text-primary-700 dark:text-primary-200 hover:bg-gradient-to-r hover:from-primary-500 hover:to-secondary-500 hover:text-white transition-all duration-300 rounded-xl mx-2">
                                <i class="fas fa-id-card mr-3"></i> Profile
                            </a>
                            <button on:click={handleLogout} class="flex items-center w-full text-left px-6 py-3 text-primary-700 dark:text-primary-200 hover:bg-gradient-to-r hover:from-primary-500 hover:to-secondary-500 hover:text-white transition-all duration-300 rounded-xl mx-2">
